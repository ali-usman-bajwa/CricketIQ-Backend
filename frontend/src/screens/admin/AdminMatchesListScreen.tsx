  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} edges={["top"]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>{matches.length} total</Text>
      </View>
        ...
        <ScrollView contentContainerStyle={styles.content}>
          {matches.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateText}>No matches have been created yet.</Text>
            </View>
          ) : (
            matches.map((m) => (
              <MatchRow key={m._id} match={m} onPress={() => navigation.navigate("AdminMatchDetail", { match: m })} />
            ))
          )}
        </ScrollView>
      )}
...
export default AdminMatchesListScreen;