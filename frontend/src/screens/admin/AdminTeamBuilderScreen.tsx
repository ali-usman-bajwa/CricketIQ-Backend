...
const LeadershipCard = ({ title, name, reason }: { title: string; name: string; reason: string }) => (
...
const AdminTeamBuilderScreen = () => {
  ...
  const openPicker = () => {
    // Admin has no single "own team" — pick from all players app-wide,
    // same pool as Comparison/AI Comparison.
    navigation.navigate("PlayerPicker", { returnTo: "TeamBuilder", minSelect: 11, maxSelect: 30 });
  };

  const groupedXI = recommendedXI
    ? ROLE_ORDER.map((role) => ({ role, players: recommendedXI.filter((e) => e.role === role) })).filter((g) => g.players.length > 0)
    : [];
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} edges={["top"]}>
      <View style={styles.header}>
        ...
        <Pressable style={({ pressed }) => [styles.pickButton, pressed && styles.pickButtonPressed]} onPress={openPicker}>
          ...
          <Text style={styles.pickButtonText}>{recommendedXI ? "Change Players" : "Select 11–30 Players"}</Text>
        </Pressable>
      </View>
      ...
      {isLoading ? (
        ...
        <View style={styles.centered}>
          ...
          <Text style={styles.emptyStateText}>Select 11–30 players and a format to get an AI-recommended XI.</Text>
        </View>
      ) : (
...
const styles = StyleSheet.create({
  ...
  pickButton: { flexDirection: "row", justifyContent: "center", alignItems: "center", gap: 6, backgroundColor: colors.accent, borderRadius: 4, paddingVertical: spacing.sm },
...
export default AdminTeamBuilderScreen;